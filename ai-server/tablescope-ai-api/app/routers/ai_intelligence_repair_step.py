...
def _repair_step_prompt(req: IntelligenceRepairSQLStepRequest) -> str:
    ...
    return (
        ...
        "casting categorical text, repeat the SELECT expression in GROUP "
        "BY, drop an unsupported function, use a column that actually "
        "exists in the queried table). If the error says an element/column "
        "is 'not defined by any relevant group', that column does NOT "
        "exist on the table in your FROM clause -- do NOT switch tables and "
        "do NOT add a JOIN; replace it with a real column listed under that "
        "SAME table, or drop that term.\n\n"
