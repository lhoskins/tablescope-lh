...
- **Vector store:** `ai-server/tablescope-ai-api/app/services/vector_store.py` — Qdrant collections are named `tablescope_tenant_{tenant_id}` (one per **tenant**, not per project), so cross-project search is physically possible without touching separate indexes. But `search_vectors()`'s current signature takes a single `project_id: int` and enforces it as a mandatory `FieldCondition`/`MatchValue` filter by design — its own docstring says "mandatory tenant + project + visibility filters... The LLM never decides what to search. The caller... provides the authenticated scope, and this function enforces it." That is a deliberate access-control invariant, not an incidental narrowing, and section 2/3.1 below treat it accordingly: the fix is a new, equally mandatory multi-project mode, not loosening the existing one.
...
- **The current user must have access to every project the routing/answering pipeline crosses into — no exceptions, and this is enforced at the query level, not filtered out of a broader result afterward.** "Cross-project" means across the projects the *asking user* already has access to within their own tenant, never across tenants and never into projects the user isn't a member of. Concretely:
  - `platform-api/app/services/ai_question_router.py` resolves `accessible_project_ids` server-side, the same way `_build_query_summary()` in `ai_proxy_ask.py` already does (owned + active-member projects via `ProjectMember`), from `context` alone — never from a client-supplied list.
  - Every retrieval call the router makes is constructed with that ID set already baked into the query, not applied as a post-filter on a wider search: the Qdrant call passes `accessible_project_ids` into a `MatchAny` filter (see 3.1), and the `BusinessInsightResult`/`HomePin` query is `WHERE tenant_id = :t AND project_id IN (:accessible_project_ids)`, not `WHERE tenant_id = :t` alone.
  - This is a strengthening of an existing invariant, not a new one: `vector_store.py::search_vectors()` already documents "the caller... provides the authenticated scope, and this function enforces it" for the single-project case. The multi-project router must uphold the identical guarantee — the vector store and the DB should never receive an unscoped, all-tenant query that relies on the caller to discard some rows afterward.
...
1. Resolve the caller's accessible `project_ids` the same way `_build_query_summary()` already does (owned + active-member projects, via `ProjectMember`), scoped to `context.tenant_id`. This list is computed once, server-side, from the authenticated request context — it is the only thing that can ever appear in the retrieval filters below, and it is never derived from or widened by anything in the question text or client payload.
...
3. Add a new `search_vectors_multi_project()` function alongside (not replacing) `search_vectors()` in `ai-server/tablescope-ai-api/app/services/vector_store.py`, taking `project_ids: list[int]` instead of a single `project_id: int`, and building its mandatory filter with Qdrant's `MatchAny(any=project_ids)` in place of `MatchValue(value=project_id)` — same tenant filter, same "the caller enforces scope" contract the existing function documents, just parameterized over a server-resolved set instead of one ID. Call it with `accessible_project_ids` from step 1. In parallel, query `BusinessInsightResult`/`HomePin` rows with `WHERE tenant_id = :t AND project_id IN (:accessible_project_ids)` for a keyword/semantic match against existing card titles/summaries/KPI tags — the access-list goes in the `WHERE` clause itself, not into a post-query filter step.
4. Combine both signals into a per-project relevance score, over the already-access-filtered candidate set only. A project with no matching vectors and no matching precomputed insight scores near zero — this is what keeps "IT" out of contention for a material-cost question, deterministically, not by hoping the LLM declines to answer from the wrong table. A project the user cannot access is never scored at all, because it was never fetched.
...
1. Build `ai_question_router.py`: accessible-project resolution (`ProjectMember`-based, server-side only), the new `search_vectors_multi_project()` call scoped to that resolved list, the access-scoped `BusinessInsightResult`/`HomePin` lookup, combined scoring, capped candidate list.
2. Add tests, at two levels — both are required, neither substitutes for the other:
   - **Query-construction level:** assert the Qdrant call is built with a `MatchAny` filter containing exactly the resolved `accessible_project_ids` (never a broader or unfiltered query that relies on trimming results afterward), and assert the SQL query's `WHERE` clause carries the same `project_id IN (...)` condition. A test double / call-spy on `search_vectors_multi_project()` should fail the build if the router ever calls it without an access-list argument.
   - **Behavioral level:** a tenant with projects across unrelated domains (mirroring the reported IT-vs-material-cost case) must rank the domain-relevant project(s) above the irrelevant one for a domain-specific question; a project the requesting user is not an active member of (or doesn't own) must never appear in the candidate list or influence its scoring, even when that project has highly relevant data — verify this with a project that would clearly win on relevance if access were ignored, and confirm it's absent.
3. Ship behind a flag, called but not yet wired into the response — verify ranking quality, access enforcement, and latency in isolation first.
...
- Router unit tests: relevance ranking correctness (including the IT-vs-cost adversarial case above), access-boundary enforcement at the query-construction level (Phase 1 item 2), a project the user isn't a member of never appearing in results even when it would clearly win on relevance, latency budget (assert the routing call makes no LLM request).
...
7. The access-boundary requirement in section 2 is load-bearing, not a nice-to-have: every new retrieval call this plan introduces (`search_vectors_multi_project()`, the tenant-wide `BusinessInsightResult`/`HomePin` lookup, and anything Phase 3-5 adds on top) must take the caller's authorized `project_ids` as a required argument baked into the query itself, mirroring the existing `search_vectors()` contract's own stated invariant ("the caller... provides the authenticated scope, and this function enforces it"). If a code review finds any new query that fetches across projects first and filters by access afterward, that is a blocking finding, not a style note.
