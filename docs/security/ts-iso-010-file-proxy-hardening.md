...
## 11. Validation addendum

All nine current-state findings in Section 2 were independently re-verified line-by-line against `platform-api/app/routes/internal_file_proxy.py`, `app/auth/middleware.py`, `app/services/safe_remote_fetch.py`, `nginx/conf.d/app.conf`, and the Java remote-file connector under `wildfly/`, and confirmed **accurate** against the current `origin/UX-design-03` tip. One finding is materially more severe in practice than Section 2's wording suggests:

- **The `X-Forwarded-For` weakness (Section 2's "Forwarded address" row) is a live, unauthenticated pre-auth bypass today, not a passive weakness.** `nginx/conf.d/app.conf` sets `X-Forwarded-For $proxy_add_x_forwarded_for`, which *appends* the real client address to whatever the caller already sent rather than replacing it. `_client_ip()` (`internal_file_proxy.py`) then trusts the *first* comma-separated value. An external, unauthenticated caller can therefore prepend any tenant's Docker CIDR (or the operator-configured `FILE_IMPORT_NETWORK_SOURCE_CIDRS` value) directly in a request header and pass `_source_allowed()` without ever touching the internal Docker network — no network position is actually required to defeat the current authorization check. Section 1's "Source IP is defense in depth, not an authorization decision" already states the right target design; this addendum is flagging that today it is worse than defense-in-depth — it is the *only* check, and it is remotely bypassable without any internal foothold. Recommend raising this specific row's urgency independent of the rest of the plan's phased rollout.

Additional findings, confirmed with file:line evidence, worth folding into Section 5/6:

- **Zero automated tests exist for this route today.** No test file references `internal_file_proxy`/`file-proxy` anywhere in `platform-api/tests/`. Section 5's "Tests" row should be split out as an early, non-blocking deliverable (a regression baseline) rather than only appearing at the end of Phase D — a hardening effort with no starting test coverage has no way to prove it hasn't regressed intermediate states.
- **The route bypasses Postgres RLS entirely, not just app-level tenant checks.** Because `/internal/file-proxy` matches the anonymous-path list, `AuthMiddleware` returns before ever establishing `rls_scope(...)` — the same DB-side tenant barrier TS-ISO-004 built for every authenticated route never applies here. Fixing the app-level `FileSourceMeta`/`NetworkFileConnection` queries to add a `tenant_id` predicate (Section 4.3, step 3/6) still leaves this route outside TS-ISO-004's defense-in-depth layer unless Section 4.1's workload-identity work explicitly also binds an `rls_scope` inside the route once a verified tenant is known.
- **`_teiid_network_cidrs()`'s hardcoded hostnames (`"teiid"`, `"tablescope-teiid-1"`) don't match the actual per-tenant naming scheme** (`tenant-{tid}-teiid`, per `tenant_layout.py`) — this looks like dead code from an earlier single-tenant topology and should be removed rather than incorporated into the hardened CIDR-comparison logic.
- **A per-tenant `teiid_api_key` secret-reference mechanism already exists (provisioned by `tenant_provisioning_service.py`, resolved by `tenant_teiid_resolver.py`) but is never used by the file proxy** — this is the natural credential to back the Java connector's already-present but currently-optional `ProxyApiKey`/`X-API-Key` support (Section 2's "Teiid adapter" row) instead of building an entirely new workload-identity/secret path from scratch. Section 6's file table should add `tenant_provisioning_service.py` and `tenant_teiid_resolver.py` as files this plan's Phase C should touch, since they already contain most of the per-tenant credential plumbing this plan needs.
