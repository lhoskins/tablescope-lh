...
All ten current-state findings in Section 2 were independently re-verified line-by-line against `platform-api/app/routes/project_assets.py`, `ai_asset_metadata.py`, `document_families_reads.py`, `app/services/ai_grounding.py`, `document_processing_service/graph.py`, `project_graph_service/{graph_primitives,lifecycle,queries}.py`, and `app/services/knowledge_graph/visibility.py`, confirmed against the actual current `origin/UX-design-03` tip (a first verification pass mistakenly checked out a local `UX-design-03` branch ref that was 106 commits stale; every finding below was re-confirmed against the real branch tip before being recorded here). Eight of the ten findings (asset content/preview, asset list, asset detail, AI profile, asset mutations, AI metadata API, project metadata/activity/aggregates, and document families/hardcoded `shared_project`) are **accurate exactly as written** — direct evidence:

- `list_assets`/`get_asset`/`get_asset_ai_profile`/`delete_asset`/`trigger_ai_processing` (`project_assets.py`) filter only by `tenant_id`/`project_id`; none call `_check_asset_read_access`. Only the content and preview routes route through `_get_readable_asset`.
- `ai_asset_metadata.py`'s tag/KPI read and mutation endpoints filter suggestion/tag/KPI rows by `tenant_id`/`project_id`/`source_type`/`source_id` only; the file never imports `ProjectAsset` or any visibility check.
- `ai_grounding.py::_lexical_project_chunks` filters `ai_document_chunks` by `tenant_id`/`project_id` only, with no join to `ai_documents`/`ProjectAsset` for owner/visibility.
- `document_processing_service/graph.py`, `project_graph_service/graph_primitives.py`, and `project_graph_service/lifecycle.py` each hardcode `'shared_project'` as the literal visibility value on every node/edge insert, regardless of the source asset's actual visibility.
- `document_families_reads.py`'s family/relationship queries (`list_document_families`, `get_document_family`, `get_document_family_members`, `_family_relationships`) filter by `tenant_id`/`project_id`/`is_active` only and take no viewer/user context — **and do not import `knowledge_graph.visibility` at all**, unlike `app/routes/project_graph.py`, which is the *only* current caller of `_hidden_project_asset_ids`. This is a concrete, named instance of Section 2's "some loaders and consumers bypass it" claim about the KG visibility filter, worth citing explicitly in Section 5.7's task list as the first migration target.

Two findings deserve a stronger, more specific characterization than Section 2's wording, in the plan's favor — the underlying protections are more robust than "Qdrant vector search... requires tenant/project plus shared-or-owned-private payload filters" and "`knowledge_graph.visibility` filters... for several graph views" convey:

- **Vector grounding search (Section 2's Qdrant row) already includes a live cross-service permission check, not just a payload filter.** `ai-server/tablescope-ai-api/app/routers/ai_grounding.py`'s `/grounding/search` endpoint calls `context_builder._verify_permissions(tenant_id, user_id, project_id)` against platform-api and builds `VectorAccessClaims` (`can_read_shared_documents`, `private_document_owner_user_id`) from the *live* result before ever querying Qdrant; `vector_store.search_vectors` then builds a `Filter(should=[...])` from those claims requiring `visibility="shared_project"` OR (`visibility` in `{private, private_project, personal}` AND `owner_user_id` matches), and raises `VectorStoreError` if the claims authorize no branch at all. This is stronger than a simple tenant/project scope check and should be credited as such in Section 5.6 — the remaining gap is specifically the "visibility generation and synchronized reindexing" language already in Section 2 (no `visibility_generation`-equivalent field was found on the Qdrant point payload), not the base authorization check itself.
- **`knowledge_graph/visibility.py` (`_hidden_project_asset_ids`) already includes an explicit tenant-admin bypass and owner/visibility join**, matching `_check_asset_read_access`'s policy exactly as its docstring claims ("Mirrors the existing per-document policy... rather than inventing a second one"). The gap is adoption breadth (one caller: `project_graph.py`), not the filter's own correctness.

No finding in Section 2 was found to be inaccurate.
