...
FROM (
  ...
  FROM {table} i
  WHERE unix_timestamp(CAST(i.opened_at AS timestamp)) >= {start} AND unix_timestamp(CAST(i.opened_at AS timestamp)) <= {end} AND i.site_code <> 'GLOBAL' AND {site_filter}
) t""",
        unit="percent",
        precision=1,
    ...
    MetricDefinition(
        ...
        date_field="resolved_at",
        numerator="resolution_minutes",
        value_expression="""SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY CAST(resolution_minutes AS double)) AS metric_value
FROM {table}
...
WHERE unix_timestamp(CAST(opened_at AS timestamp)) <= {end} - 2592000
  ...
  AND {site_filter}""",
    ...
    MetricDefinition(
        ...
        table="01_incidents_CSV",
        date_field="opened_at",
        value_expression="""SELECT AVG(({end} - unix_timestamp(CAST(opened_at AS timestamp))) / 86400.0) AS metric_value
FROM {table}
...
FROM (
  ...
  WHERE sd.\"type\" IN ('Resolution', 'resolution')
    AND unix_timestamp(CAST(i.resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(i.resolved_at AS timestamp)) <= {end}
    AND {site_filter}
) t""",
        unit="percent",
        precision=1,
...
FROM (
  ...
  JOIN "03_sla_definitions_CSV" sd ON sl.sla_definition_sys_id = sd.sys_id
  JOIN "01_incidents_CSV" i ON sl.task_sys_id = i.sys_id AND sl.task_type = 'Incident'
  WHERE unix_timestamp(CAST(i.resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(i.resolved_at AS timestamp)) <= {end}
    AND {site_filter}
) t""",
        unit="percent",
        precision=1,
...
_SERVICE_REQUEST_METRICS: list[MetricDefinition] = [
    ...
    MetricDefinition(key="request_sla", label="Request SLA", dashboard="service_request", order=6, kind="ratio_period", table="07_requests_CSV", date_field="closed_at", value_expression="""SELECT CASE WHEN total > 0 THEN 100.0 * met / total ELSE 0 END AS metric_value FROM (SELECT COUNT(DISTINCT sys_id) AS total, COUNT(DISTINCT CASE WHEN fulfillment_sla_met = true THEN sys_id END) AS met FROM {table} WHERE unix_timestamp(CAST(closed_at AS timestamp)) >= {start} AND unix_timestamp(CAST(closed_at AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=1, polarity="higher_is_better"),
...
_AVAILABILITY_METRICS: list[MetricDefinition] = [
    MetricDefinition(key="estimated_availability", label="Estimated availability", dashboard="availability", order=1, kind="ratio_period", table="15_service_outages_CSV", date_field="begin", value_expression="""SELECT CASE WHEN total_minutes > 0 THEN GREATEST(0, 100.0 - (100.0 * unplanned_minutes / total_minutes)) ELSE 100.0 END AS metric_value FROM (SELECT COALESCE(SUM(CASE WHEN planned = false THEN duration_minutes END), 0) AS unplanned_minutes, COALESCE(SUM(duration_minutes), 0) AS total_minutes FROM {table} WHERE unix_timestamp(CAST(begin AS timestamp)) >= {start} AND unix_timestamp(CAST(begin AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=3, polarity="higher_is_better", target=99.9),
...
_PROBLEM_METRICS: list[MetricDefinition] = [
    ...
    MetricDefinition(key="root_cause_rate", label="Root-cause rate", dashboard="problem", order=6, kind="ratio_period", table="04_problems_CSV", date_field="resolved_at", value_expression="""SELECT CASE WHEN total > 0 THEN 100.0 * with_root_cause / total ELSE 0 END AS metric_value FROM (SELECT COUNT(DISTINCT sys_id) AS total, COUNT(DISTINCT CASE WHEN root_cause IS NOT NULL THEN sys_id END) AS with_root_cause FROM {table} WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=1, polarity="higher_is_better"),
    MetricDefinition(key="known_error_rate", label="Known-error rate", dashboard="problem", order=7, kind="ratio_period", table="04_problems_CSV", date_field="resolved_at", value_expression="""SELECT CASE WHEN total > 0 THEN 100.0 * known / total ELSE 0 END AS metric_value FROM (SELECT COUNT(DISTINCT sys_id) AS total, COUNT(DISTINCT CASE WHEN known_error = true THEN sys_id END) AS known FROM {table} WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=1, polarity="higher_is_better"),
    MetricDefinition(key="workaround_coverage", label="Workaround coverage", dashboard="problem", order=8, kind="ratio_period", table="04_problems_CSV", date_field="resolved_at", value_expression="""SELECT CASE WHEN total > 0 THEN 100.0 * with_workaround / total ELSE 0 END AS metric_value FROM (SELECT COUNT(DISTINCT sys_id) AS total, COUNT(DISTINCT CASE WHEN workaround IS NOT NULL THEN sys_id END) AS with_workaround FROM {table} WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=1, polarity="higher_is_better"),
