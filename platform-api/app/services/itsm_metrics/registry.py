"""Central ITSM metric registry for ServiceNow KPI and insight dashboard presets.
...
FROM (
  SELECT COUNT(i.sys_id) AS incident_count,
         (SELECT MAX(CAST(bs.employee_population AS double)) FROM "14_business_services_CSV" bs) AS population
  FROM {table} i
  WHERE unix_timestamp(CAST(i.opened_at AS timestamp)) >= {start} AND unix_timestamp(CAST(i.opened_at AS timestamp)) <= {end} AND {site_filter}
) t""",
        unit="percent",
        precision=1,
    ...
    MetricDefinition(
        ...
        description="The midpoint open-to-resolution duration for incidents resolved during the reporting period.",
        calculation="Median open-to-resolution duration.",
    ),
    MetricDefinition(
        key="mean_restore",
        label="Mean restore",
...
FROM (
  SELECT
    COUNT(CASE WHEN state IN ('Resolved', 'Closed') THEN sys_id END) AS resolved_count,
    COUNT(CASE WHEN state IN ('Resolved', 'Closed') AND CAST(reopen_count AS integer) = 0 AND CAST(reassign_count AS integer) = 0 THEN sys_id END) AS fcr_count
  FROM {table}
  WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}
) t""",
        unit="percent",
        precision=1,
...
FROM (
  SELECT COUNT(sys_id) AS total,
         COUNT(CASE WHEN CAST(reassign_count AS integer) > 0 THEN sys_id END) AS reassigned
  FROM {table}
  WHERE unix_timestamp(CAST(opened_at AS timestamp)) >= {start} AND unix_timestamp(CAST(opened_at AS timestamp)) <= {end} AND {site_filter}
) t""",
        unit="percent",
        precision=1,
        polarity="lower_is_better",
...
FROM (
  SELECT COUNT(CASE WHEN state IN ('Resolved', 'Closed') THEN sys_id END) AS resolved_count,
         COUNT(CASE WHEN state IN ('Resolved', 'Closed') AND CAST(reopen_count AS integer) > 0 THEN sys_id END) AS reopened
  FROM {table}
  WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}
) t""",
        unit="percent",
    ...
    MetricDefinition(
        ...
        polarity="lower_is_better",
        drill_down_dimensions=["site_code", "priority"],
        description="Number of priority-one or major incidents created during the reporting period.",
        calculation="Number of priority-one/major incidents.",
    ),
    MetricDefinition(
    ...
    MetricDefinition(
        ...
        description="Incidents that remain unresolved at the end of the reporting period.",
        calculation="Count of unresolved incidents.",
    ),
    MetricDefinition(
        ...
        date_field="opened_at",
        state_field="state",
        value_expression="""SELECT COUNT(sys_id) AS metric_value
FROM {table}
WHERE unix_timestamp(CAST(opened_at AS timestamp)) <= {end} - 2592000
  AND (unix_timestamp(CAST(resolved_at AS timestamp)) IS NULL OR unix_timestamp(CAST(resolved_at AS timestamp)) > {end})
  AND {site_filter}""",
        unit="count",
        precision=0,
        polarity="lower_is_better",
    ...
    MetricDefinition(
        ...
        table="01_incidents_CSV",
        date_field="opened_at",
        value_expression="""SELECT AVG(CAST(({end} - unix_timestamp(CAST(opened_at AS timestamp))) / 86400.0 AS double)) AS metric_value
FROM {table}
...
FROM (
  SELECT COUNT(sl.sys_id) AS total,
         COUNT(CASE WHEN CAST(sl.has_breached AS boolean) = true THEN sl.sys_id END) AS breached
  FROM "02_task_slas_CSV" sl
  WHERE sl.task_type = 'Incident'
    AND sl.\"metric\" IN ('Resolution')
    AND unix_timestamp(CAST(sl.end_time AS timestamp)) >= {start} AND unix_timestamp(CAST(sl.end_time AS timestamp)) <= {end}
    AND {site_filter}
) t""",
        unit="percent",
        precision=1,
...
FROM (
  SELECT COUNT(CASE WHEN state IN ('Resolved', 'Closed') THEN sys_id END) AS resolved_count,
         COUNT(CASE WHEN state IN ('Resolved', 'Closed') AND CAST(knowledge_used AS boolean) = true THEN sys_id END) AS with_knowledge
  FROM {table}
  WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}
) t""",
        unit="percent",
...
_SERVICE_REQUEST_METRICS: list[MetricDefinition] = [
    ...
    MetricDefinition(key="backlog_older_than_30_days_requests", label="Backlog older than 30 days", dashboard="service_request", order=8, kind="snapshot_eom", table="07_requests_CSV", date_field="requested_date", value_expression="""SELECT COUNT(sys_id) AS metric_value FROM {table} WHERE unix_timestamp(CAST(requested_date AS timestamp)) <= {end} - 2592000 AND (closed_at IS NULL OR unix_timestamp(CAST(closed_at AS timestamp)) > {end}) AND {site_filter}""", unit="count", precision=0, polarity="lower_is_better", status="proxy"),
    ...
    MetricDefinition(key="open_catalog_tasks", label="Open catalog tasks", dashboard="service_request", order=10, kind="snapshot_eom", table="09_catalog_tasks_CSV", date_field="opened_at", close_field="closed_at", aggregation="distinct", unit="count", precision=0, polarity="lower_is_better"),
    MetricDefinition(key="overdue_tasks", label="Overdue tasks", dashboard="service_request", order=11, kind="snapshot_eom", table="09_catalog_tasks_CSV", date_field="opened_at", value_expression="SELECT NULL AS metric_value", unit="count", precision=0, polarity="lower_is_better", status="not_implemented"),
    ...
    MetricDefinition(key="catalog_value", label="Catalog value", dashboard="service_request", order=13, kind="event_period", table="08_requested_items_CSV", date_field="opened_at", aggregation="sum", numerator="price_usd", unit="currency", precision=0, polarity="neutral", status="proxy"),
    MetricDefinition(key="value_per_fulfilled_request", label="Value per fulfilled request", dashboard="service_request", order=14, kind="ratio_period", table="08_requested_items_CSV", date_field="closed_at", value_expression="SELECT NULL AS metric_value", unit="currency", precision=0, polarity="neutral", status="not_implemented"),
    MetricDefinition(
        key="automated_fulfillment_rate",
        label="Automated fulfillment",
        dashboard="service_request",
        order=15,
        kind="ratio_period",
        table="08_requested_items_CSV",
        date_field="closed_at",
        value_expression="""SELECT CASE WHEN fulfilled > 0 THEN CAST(100.0 * automated / fulfilled AS double) ELSE 0 END AS metric_value
FROM (
  SELECT COUNT(ri.sys_id) AS fulfilled,
         COUNT(CASE WHEN COALESCE(task_counts.task_count, 0) <= 1 THEN ri.sys_id END) AS automated
  FROM {table} ri
  LEFT JOIN (
    SELECT request_item_sys_id, COUNT(sys_id) AS task_count
    FROM "09_catalog_tasks_CSV"
    GROUP BY request_item_sys_id
  ) task_counts ON task_counts.request_item_sys_id = ri.sys_id
  WHERE unix_timestamp(CAST(ri.closed_at AS timestamp)) >= {start}
    AND unix_timestamp(CAST(ri.closed_at AS timestamp)) <= {end}
    AND {site_filter}
) t""",
        unit="percent",
        precision=1,
...
_PRODUCTIVITY_METRICS: list[MetricDefinition] = [
    ...
    MetricDefinition(key="tickets_per_analyst", label="Tickets per analyst", dashboard="productivity", order=2, kind="ratio_period", table="01_incidents_CSV", date_field="opened_at", value_expression="SELECT NULL AS metric_value", unit="count", precision=1, polarity="neutral", status="not_implemented"),
    MetricDefinition(key="resolutions_per_analyst", label="Resolutions per analyst", dashboard="productivity", order=3, kind="ratio_period", table="01_incidents_CSV", date_field="resolved_at", value_expression="SELECT NULL AS metric_value", unit="count", precision=1, polarity="higher_is_better", status="not_implemented"),
    MetricDefinition(key="resolution_rate", label="Resolution rate", dashboard="productivity", order=4, kind="ratio_period", table="01_incidents_CSV", date_field="opened_at", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="higher_is_better", status="not_implemented"),
    MetricDefinition(key="backlog_per_analyst", label="Backlog per analyst", dashboard="productivity", order=5, kind="ratio_period", table="01_incidents_CSV", date_field="opened_at", value_expression="SELECT NULL AS metric_value", unit="count", precision=1, polarity="lower_is_better", status="not_implemented"),
    ...
    MetricDefinition(key="resolution_sla_prod", label="Resolution SLA", dashboard="productivity", order=8, kind="ratio_period", table="01_incidents_CSV", date_field="resolved_at", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="higher_is_better", status="not_implemented"),
    MetricDefinition(key="fcr_proxy_prod", label="FCR proxy", dashboard="productivity", order=9, kind="ratio_period", table="01_incidents_CSV", date_field="resolved_at", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="higher_is_better", status="not_implemented"),
    MetricDefinition(key="transfer_proxy", label="Transfer proxy", dashboard="productivity", order=10, kind="ratio_period", table="01_incidents_CSV", date_field="resolved_at", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="lower_is_better", status="not_implemented"),
    MetricDefinition(key="knowledge_reuse_prod", label="Knowledge reuse", dashboard="productivity", order=11, kind="ratio_period", table="01_incidents_CSV", date_field="resolved_at", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="higher_is_better", status="not_implemented"),
    MetricDefinition(key="reopen_rate_prod", label="Reopen rate", dashboard="productivity", order=12, kind="ratio_period", table="01_incidents_CSV", date_field="resolved_at", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="lower_is_better", status="not_implemented"),
    MetricDefinition(key="self_service_intake", label="Self-service intake", dashboard="productivity", order=13, kind="ratio_period", table="07_requests_CSV", date_field="requested_date", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="higher_is_better", status="not_implemented"),
    MetricDefinition(key="monitoring_intake", label="Monitoring intake", dashboard="productivity", order=14, kind="ratio_period", table="01_incidents_CSV", date_field="opened_at", value_expression="SELECT NULL AS metric_value", unit="percent", precision=1, polarity="lower_is_better", status="not_implemented"),
]
...
_PROBLEM_METRICS: list[MetricDefinition] = [
    ...
    MetricDefinition(key="root_cause_rate", label="Root-cause rate", dashboard="problem", order=6, kind="ratio_period", table="04_problems_CSV", date_field="resolved_at", value_expression="""SELECT CASE WHEN total > 0 THEN CAST(100.0 * with_root_cause / total AS double) ELSE 0 END AS metric_value FROM (SELECT COUNT(sys_id) AS total, COUNT(CASE WHEN root_cause_summary IS NOT NULL THEN sys_id END) AS with_root_cause FROM {table} WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=1, polarity="higher_is_better"),
    MetricDefinition(key="known_error_rate", label="Known-error rate", dashboard="problem", order=7, kind="ratio_period", table="04_problems_CSV", date_field="resolved_at", value_expression="""SELECT CASE WHEN total > 0 THEN CAST(100.0 * known / total AS double) ELSE 0 END AS metric_value FROM (SELECT COUNT(sys_id) AS total, COUNT(CASE WHEN CAST(known_error AS boolean) = true THEN sys_id END) AS known FROM {table} WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=1, polarity="higher_is_better"),
    MetricDefinition(key="workaround_coverage", label="Workaround coverage", dashboard="problem", order=8, kind="ratio_period", table="04_problems_CSV", date_field="resolved_at", value_expression="""SELECT CASE WHEN total > 0 THEN CAST(100.0 * with_workaround / total AS double) ELSE 0 END AS metric_value FROM (SELECT COUNT(sys_id) AS total, COUNT(CASE WHEN workaround_summary IS NOT NULL THEN sys_id END) AS with_workaround FROM {table} WHERE unix_timestamp(CAST(resolved_at AS timestamp)) >= {start} AND unix_timestamp(CAST(resolved_at AS timestamp)) <= {end} AND {site_filter}) t""", unit="percent", precision=1, polarity="higher_is_better"),
    ...
    MetricDefinition(key="fix_in_progress", label="Fix in progress", dashboard="problem", order=14, kind="snapshot_eom", table="04_problems_CSV", date_field="opened_at", filters=[FilterSpec(column="state", operator="in", value=["Fix in Progress", "Work in Progress"])], aggregation="distinct", unit="count", precision=0, polarity="neutral", status="proxy"),
]
...
_CHARTS: dict[str, list[tuple[str, str, str]]] = {
    ...
    "service_request": [
        ...
        ("catalog_demand_by_category", "Catalog demand by category", "catalog_item_category"),
    ],
    "availability": [
    ...
    "problem": [
        ...
        ("root_cause_distribution", "Root-cause distribution", "root_cause_summary"),
    ],
}
...
_REGISTRY: dict[str, list[MetricDefinition]] = {
    ...
    "incident_insights": [
        MetricDefinition(
            **{
                **next(metric for metric in _INCIDENT_METRICS if metric.key == key).__dict__,
    ...
    "service_request_insights": [
        MetricDefinition(
            **{
                **next(metric for metric in _SERVICE_REQUEST_METRICS if metric.key == key).__dict__,
                "dashboard": "service_request_insights",
                "order": order,
                "group_by": None,
                "chart_label": None,
            }
        ...
        for order, key in enumerate(
            ("request_backlog", "request_sla", "median_fulfillment", "automated_fulfillment_rate"),
            start=1,
        )
    ],
}
