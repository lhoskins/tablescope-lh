...
class OllamaAdapter:
    ...
    The GGUF is staged locally on the deployment worker, uploaded to the target
    Ollama host by digest via ``/api/blobs/:digest``, and then imported with
    ``/api/create`` using a ``files`` reference. This avoids requiring a shared
    filesystem between the worker and the target runtime.
    """
    ...
    async def _upload_blob(self, fs_path: Path) -> str:
        ...
        async with httpx.AsyncClient(timeout=upload_timeout) as client:
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_502_BAD_GATEWAY,
                    detail=f"Ollama blob upload returned {exc.response.status_code}: {exc.response.text[:200]}",
                ) from exc
            except httpx.RequestError as exc:
                raise HTTPException(
                    status_code=status.HTTP_504_GATEWAY_TIMEOUT,
                    detail=f"Could not upload blob to Ollama at {self.base_url}: {exc}",
                ) from exc
        return digest

    def _sha256(self, fs_path: Path) -> str:
    ...
    def _target_paths(self, artifact_id: int, filename: str) -> Path:
        """Return the local staging path for the GGUF before it is uploaded."""
    ...
    def _build_create_body(self, model_name: str, digest: str) -> dict[str, Any]:
        return {
            "model": model_name,
            "files": {"model.gguf": digest},
            "stream": False,
            "parameters": {"stop": ["User:", "Assistant:"]},
        }
    ...
    async def install(
        ...
        """Stage, upload and create the model in the remote Ollama runtime."""
        ...
        fs_path = self._target_paths(artifact_id, source.name)
        ...
        try:
            ...
            return InstallResult(
                ...
                installed_path=None,
                modelfile_content=None,
                detail=f"Could not copy GGUF to staging path: {exc}",
            )

        model_name = f"tablescope-{artifact_id}-{_sanitize_model_name(artifact_name)}"

        try:
            digest = await self._upload_blob(fs_path)
        except HTTPException as exc:
            return InstallResult(
                success=False,
        ...
        create_body = self._build_create_body(model_name, digest)

        try:
            await self._request("POST", "/api/create", json=create_body)
        except HTTPException as exc:
            return InstallResult(
                success=False,
                ollama_model_name=model_name,
                installed_path=str(fs_path),
                modelfile_content=str(create_body),
                detail=str(exc.detail),
            )

        # Verify the model appears in the target's tag list.
        tags_response = await self._request("GET", "/api/tags")
