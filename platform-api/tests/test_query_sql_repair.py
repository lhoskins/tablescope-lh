...
burning the full repair budget (multiple Teiid round trips plus fix-sql LLM
...
async def _run_repair(monkeypatch, *, run_sql_error: str, fix_sql_return: str | None = "SELECT 1"):
    ...
    async def fake_fix_sql(**kwargs):
        fix_sql_calls.append(kwargs["sql"])
        return fix_sql_return

    monkeypatch.setattr("app.routes.query_sql_helpers._run_sql", fake_run_sql)
    monkeypatch.setattr(
        "app.routes.query_sql_helpers.ai_intelligence_client.fix_sql", fake_fix_sql
    )
    ...
    return result, run_sql_calls, fix_sql_calls
...
async def test_dialect_and_parse_errors_fail_fast_without_llm_repair(monkeypatch, error):
    result, run_sql_calls, fix_sql_calls = await _run_repair(monkeypatch, run_sql_error=error)

    assert result is None
    # One Teiid round trip, not the full max_attempts budget -- and no
    # fix-sql LLM call, since these error classes keep recurring unchanged.
...
async def test_other_errors_still_go_through_the_full_repair_loop(monkeypatch):
    result, run_sql_calls, fix_sql_calls = await _run_repair(
        monkeypatch, run_sql_error="TEIID30999 some transient issue"
    )

    assert result is None
    assert len(run_sql_calls) == 3
    assert len(fix_sql_calls) == 2
