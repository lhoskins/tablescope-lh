...
function search(term, projectIds = null) {
  ...
  // get recent
  if (!term) {
    return Query.recent(params).$promise
      .then((results) => {
  ...
  // search by query
  params.q = term;
  return Query.query(params).$promise
    .then(({ results }) => Promise.resolve(results));
}

export function QuerySelector(props) {
  const [searchTerm, setSearchTerm] = useState(''); // Initialize with empty string to trigger initial load
  ...
  // Load initial queries when projectIds change
  useEffect(() => {
    if (!selectedQuery) {
      setSearchTerm(''); // Trigger search with empty term to load recent queries
    }
  }, [props.projectIds]);
...
QuerySelector.propTypes = {
  ...
  projectIds: PropTypes.array, // eslint-disable-line react/forbid-prop-types
};
