...
import tsqueryTemplate from './TSqueryCode.html';

// ──────────────── FIX: Hoisted to global scope ────────────────
function detectSourceTable(sqlText = '') {
  // flexible, case-insensitive FROM extractor
  const regex = /\bfrom\s+[`"'[\]]*([A-Za-z0-9_.]+)[`"'[\]]*/i;
  console.log('TSviewCode.detectSourceTable input:', sqlText);
  const match = regex.exec(sqlText);
  console.log('TSviewCode.detectSourceTable match:', match);
...
function QueryViewCtrl(
  ...
  /* ────────────────────────────────────────────────────────────────
     TS Workspace patch - make all links point at /tcqueries/…
     ──────────────────────────────────────────────────────────────── */
  if (!Query.__tsWorkspacePatched) {
    Query.__tsWorkspacePatched = true;

    const orgSlug = () => (window.location.pathname.split('/')[1] || 'default');

    // Override the helpers used everywhere in the UI:
    Query.prototype.getUrl = function getUrl(sourceMode, tab) {
      const path = `/tcqueries/${this.id}${sourceMode ? '/source' : ''}`;
      return `/${orgSlug()}${path}${tab ? '#' + tab : ''}`;
    };

    Query.prototype.getSourceLink = function getSourceLink() {
      return `/${orgSlug()}/tcqueries/${this.id}/source`;
    };
  }
  /* ─────────────────────────────────────────────────────────────── */
  ...
  function getDataSourceId() {
    ...
    const isValid = !Number.isNaN(dataSourceId) && some($scope.dataSources, ds => ds.id === dataSourceId);
    if (!isValid) {
  ...
  function updateDataSources(dataSources) {
    function canUse(ds) {
      return !ds.view_only || ds.id === $scope.query.data_source_id;
    }
    $scope.dataSources = dataSources.filter(canUse);
  ...
  // --- next chunk will continue from canForkQuery ---
  ...
  // --- next chunk continues with status watch & export ---
  $scope.$watch('queryResult && queryResult.getStatus()', (status) => {
    ...
    if (status === 'done') {
      ...
      // No dispatch here; TSview.js handles event dispatch
  ...
  $scope.$watch(
    ...
    (hash) => {
      $scope.selectedVisualization =
        find($scope.query.visualizations, item => item.id === hash) ||
...
export default function init(ngModule) {
  ngModule.controller('TSQueryViewCtrl', QueryViewCtrl); // same controller or custom one
  return {
    '/tcqueries/:queryId': {
