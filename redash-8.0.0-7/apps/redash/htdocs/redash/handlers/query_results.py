...
error_messages = {
    ...
    'no_query_access': error_response('You do not have permission to execute this query.', 403),
    'no_datasource_access': error_response('You do not have permission to access the data source for this query.', 403),
...
class QueryResultListResource(BaseResource):
    ...
    def post(self):
        ...
        # Set user context for VDB routing (will route to user VDB if available)
        from flask import g
        g.user = self.current_user
        
        # Set project context for VDB routing (will route to shared VDB if query belongs to shared project)
        if query_id and query_id != 'adhoc':
            try:
                query_obj = models.Query.get_by_id_and_org(query_id, self.current_org)
                if query_obj and query_obj.project_id and len(query_obj.project_id) > 0:
                    # Query belongs to a project, set project context
                    project_id = query_obj.project_id[0]  # Use first project if multiple
                    project = models.Project.query.filter_by(id=project_id).first()
                    if project:
                        g.project = project
                        logger.info("Set project context for query execution: project_id={}".format(project_id))
            except Exception as e:
                logger.warning("Could not set project context: {}".format(str(e)))
...
class QueryResultResource(BaseResource):
    ...
    def post(self, query_id):
        ...
        # Use standard has_access check which handles admin permissions and group access
        ...
        if query_result:
            ...
            if query:
                ...
                if has_access(query, self.current_user, allow_executing_with_view_only_permissions):
                    ...
                    return run_query(query.parameterized, parameter_values, query.data_source, query.id)
                else:
                    if not query.parameterized.is_safe:
                        if current_user.is_api_user():
                            return error_messages['unsafe_when_shared']
