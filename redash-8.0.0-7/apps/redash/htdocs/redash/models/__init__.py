...
# Inside redash/models/__init__.py
from .project import Project, ProjectMember, ProjectDataSource
from .data_source_approval_model import DataSourceApproval
from .table_scope import TableScope
from .organization_vdb import OrganizationVDB
from .user_vdb import UserVDB
from .shared_vdb import SharedVDB
...
from .data_migration_log import DataMigrationLog
...
class DataSource(BelongsToOrgMixin, db.Model):
    ...
    owner = Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # New owner field
    owner_user = relationship("User", backref="owned_data_sources", foreign_keys=[owner])
    
    # Shared project data migration fields (Requirements: 3.1, 3.2, 3.3, 3.4, 3.5)
    is_shared = Column(db.Boolean, default=False)  # Indicates if datasource is currently shared
    shared_at = Column(db.DateTime(True), nullable=True)  # Timestamp when datasource was shared
    private_file_path = Column(db.Text, nullable=True)  # Original file path in user private directory
    shared_file_path = Column(db.String(512), nullable=True)  # Path to shared file when project is shared
    original_owner_id = Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # Original owner when shared
    original_owner = relationship("User", backref="originally_owned_data_sources", foreign_keys=[original_owner_id])

    # Relationships
    data_source_groups = relationship("DataSourceGroup", back_populates="data_source", cascade="all")
    project_data_sources = relationship("ProjectDataSource", back_populates="data_source", cascade="all, delete-orphan")
    data_source_approvals = relationship("DataSourceApproval", back_populates="data_source", cascade="all, delete-orphan")
    ...
    def to_dict(self, all=False, with_permissions_for=None):
        d = {
            ...
            'name': self.name,
            'type': self.type,
            'syntax': self.query_runner.syntax if self.query_runner else None,  # Prevent AttributeError
            'paused': self.paused,
            'pause_reason': self.pause_reason,
            'owner': self.owner,  # Include owner in the serialized output
            'shared_file_path': self.shared_file_path,
            'original_owner_id': self.original_owner_id,
        }

        # Ensure query runner exists before accessing configuration schema
        if all:
            schema = get_configuration_schema_for_query_runner_type(self.type) if self.query_runner else {}
        ...
        # Check for permission query only if DataSourceGroup entry exists
        if with_permissions_for is not None:
            permission_query = db.session.query(DataSourceGroup.view_only).filter(
                DataSourceGroup.group == with_permissions_for,
                DataSourceGroup.data_source == self
            ).first()  # Use `.first()` to prevent `NoneType` errors

            d['view_only'] = permission_query[0] if permission_query else False  # Default to False if no permission
    ...
    def create_with_group(cls, *args, **kwargs):
        owner_id = kwargs.pop('owner', None)  # Extract owner from kwargs
        data_source = cls(*args, **kwargs)
        data_source.owner = owner_id  # Assign the owner to the data source
        
        # Auto-populate private_file_path from options if available
        # This ensures the migration process can find the file path
        if data_source.options and isinstance(data_source.options, dict):
            file_path = None
            
            # Try different possible keys for file path
            if 'file' in data_source.options:
                file_path = data_source.options['file']
            elif 'path' in data_source.options:
                file_path = data_source.options['path']
            elif 'file_path' in data_source.options:
                file_path = data_source.options['file_path']
            
            # Set private_file_path if we found a file path
            if file_path and not data_source.private_file_path:
                data_source.private_file_path = file_path
                
                import logging
                logger = logging.getLogger(__name__)
                logger.info("Auto-populated private_file_path for datasource '{}': {}".format(
                    data_source.name, file_path
                ))
        ...
        # Trigger lifecycle hook for data source creation
        try:
            from redash.services.vdb_lifecycle_hooks import on_data_source_created
            on_data_source_created(data_source)
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.error('Failed to execute data source creation hook: {}'.format(str(e)))
    ...
    # Ownership metadata methods for user-level VDB isolation
    def get_original_owner(self):
        """
        Get the original owner of the data source.
        
        Returns:
            User: The original owner user object, or None if not shared
        """
        if self.original_owner_id:
            return self.original_owner
        return None
    
    def is_datasource_shared(self):
        """
        Check if the data source is shared based on file path.
        
        A data source is considered shared if it has a shared_file_path set.
        This method is different from the is_shared column which tracks migration status.
        
        Returns:
            bool: True if data source is shared, False otherwise
        """
        return self.shared_file_path is not None
    
    def get_file_path(self):
        """
        Get the appropriate file path for the data source.
        
        Returns the shared_file_path if the data source is shared,
        otherwise returns the private file path from options.
        
        Returns:
            str: The file path to use for this data source
        """
        if self.is_datasource_shared():
            return self.shared_file_path
        
        # Return private file path from options if available
        if self.options and hasattr(self.options, 'get'):
            return self.options.get('file_path')
    ...
    def current_file_path(self):
        """
        Get the current active file path based on sharing status.
        
        Requirements: 3.1, 3.2, 3.3, 3.4, 3.5
        
        Returns:
            str: The shared_file_path if datasource is shared, otherwise private_file_path
        """
        return self.shared_file_path if self.is_shared else self.private_file_path
    
    def mark_as_shared(self, shared_path):
        """
        Mark datasource as shared and update paths.
        
        Requirements: 3.1, 3.2, 3.3, 3.4, 3.5
        
        Args:
            shared_path (str): The new shared file path
        """
        self.is_shared = True
        self.original_owner_id = self.owner
        self.shared_at = db.func.now()
        self.shared_file_path = shared_path
    
    def mark_as_private(self, private_path):
        """
        Mark datasource as private and update paths.
        
        Requirements: 3.1, 3.2, 3.3, 3.4, 3.5
        
        Args:
            private_path (str): The new private file path
        """
        self.is_shared = False
        self.shared_at = None
        self.private_file_path = private_path
...
# SQLAlchemy event listener to auto-populate private_file_path
# This ensures that datasources created by external systems (like Wildfly servlet)
# automatically have their private_file_path populated from the options JSON
@listens_for(DataSource, 'before_insert')
@listens_for(DataSource, 'before_update')
def auto_populate_private_file_path(mapper, connection, target):
    """
    Automatically populate private_file_path from options when datasource is created or updated.
    
    This ensures that the migration process can find file paths even for datasources
    created by external systems that don't explicitly set private_file_path.
    
    Requirements: 2.1, 2.2, 2.3
    """
    # Only populate if private_file_path is not already set
    if not target.private_file_path and target.options:
        file_path = None
        
        # Try different possible keys for file path in options
        if isinstance(target.options, dict):
            if 'file' in target.options:
                file_path = target.options['file']
            elif 'path' in target.options:
                file_path = target.options['path']
            elif 'file_path' in target.options:
                file_path = target.options['file_path']
        elif hasattr(target.options, 'get'):
            # ConfigurationContainer has a get method
            file_path = target.options.get('file') or target.options.get('path') or target.options.get('file_path')
        
        # Set private_file_path if we found a file path
        if file_path:
            target.private_file_path = file_path
            logger.info("Auto-populated private_file_path for datasource '{}': {}".format(
                target.name if hasattr(target, 'name') else target.id, file_path
            ))
...
class QueryResult(db.Model, BelongsToOrgMixin):
    ...
    def store_result(cls, org, data_source, query_hash, query, data, run_time, retrieved_at):
        ...
        logging.info("CACHE DEBUG: Found %s queries matching hash=%s, data_source_id=%s", 
                    query_count, query_hash, data_source.id)
        for q in queries:
            logging.info("CACHE DEBUG: Updating query id=%s with result id=%s", q.id, query_result.id)
...
class Query(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    project_id = Column(ARRAY(db.Integer), nullable=True)  # New field for multiple projects
    ...
    owner = Column(db.Integer, nullable=True)  # Owner (nullable, no FK)
    ...
    # Shared project data migration fields (Requirements: 8.1, 8.2, 8.3, 8.4)
    is_shared = Column(db.Boolean, default=False)  # Indicates if query is currently shared
    shared_at = Column(db.DateTime(True), nullable=True)  # Timestamp when query was shared
    ...
    def to_dict(self):
        """
        Convert the Query instance into a dictionary for logging and debugging.
        """
        return {
            'id': self.id,
            'name': self.name,
            'description': self.description,
            'project_id': self.project_id,  # Include project IDs
...
class Dashboard(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    project_id = Column(db.Integer, db.ForeignKey("projects.id"), nullable=True)  # New field
    project = db.relationship("Project", backref="dashboards")  # Relationship to Project model
    ...
    # ------------------------------------------------------------------
    def to_dict(self):
        """Minimal dictionary serialization of the Dashboard object."""
        return {
            "id": self.id,
...
def init_db():
    ...
    admin_group = Group(name='admin', permissions=['admin', 'super_admin'], org=default_org, type=Group.BUILTIN_GROUP, role_type='org_admin')
    default_group = Group(name='default', permissions=Group.DEFAULT_PERMISSIONS, org=default_org, type=Group.BUILTIN_GROUP, role_type=None)
