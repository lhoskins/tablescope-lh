...
Stores VDB (Virtual Database) mappings and credentials for organization-wide shared projects.
Provides organization-level shared data access through dedicated Teiid VDBs.
"""
...
class SharedVDB(TimestampMixin, db.Model):
    """
    Model for storing VDB configuration and credentials for organization-wide shared projects.
    
    Each organization gets a shared VDB with unique credentials for collaborative data access.
    """
    ...
    organization = relationship('Organization', backref='shared_vdb_config')
    ...
    def __str__(self):
        return u'SharedVDB(org_id={}, vdb_id={})'.format(self.organization_id, self.vdb_id)
    
    @classmethod
    def get_by_organization(cls, org_id):
        """Get shared VDB configuration for an organization."""
    ...
    def get_connection_string(self):
        """
        Build PostgreSQL-compatible connection string for this shared VDB.
        ...
        """
        # Teiid requires VDB name with version (e.g., "9162153.1")
    ...
    def update_health_status(self, status, check_time=None):
        """Update the health status of this shared VDB."""
