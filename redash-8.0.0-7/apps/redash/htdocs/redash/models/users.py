...
class User(TimestampMixin, db.Model, BelongsToOrgMixin, UserMixin, PermissionsCheckMixin):
    ...
    org = db.relationship("Organization", foreign_keys=[org_id], backref=db.backref("users", lazy="dynamic"))
    ...
    # Add this relationship to link users to their owned groups
    owned_groups = relationship("Group", back_populates="owner", foreign_keys="[Group.owner_id]")
        # Add the owned_projects relationship
...
class Group(db.Model, BelongsToOrgMixin):
    DEFAULT_PERMISSIONS = ['create_dashboard', 'create_query', 'edit_dashboard', 'edit_query',
                           ...
                           'list_dashboards', 'list_alerts', 'list_data_sources', 'create_project']
    ...
    ROLE_PERMISSIONS = {
        ROLE_DEFAULT: [
            ...
            'create_project',
            'edit_own_datasource',
            'delete_own_datasource',
            'edit_own_query',
            'delete_own_query',
            'execute_own_query',
            'edit_own_project',
            'delete_own_project',
            'manage_own_project_members',
            'view_own_resources'
        ],
        ROLE_DESIGNER: [
            'create_datasource',
            ...
            'assign_own_datasource_to_project'
        ],
        ROLE_PROJECT_OWNER: [
            'create_datasource',
            'create_query',
            ...
            'edit_project',
            'delete_project',
            'assign_datasource_to_project',
            'remove_datasource_from_project'
        ],
        ROLE_PROJECT_ADMIN: [
            'create_datasource',
            'create_query',
    ...
    id = Column(db.Integer, primary_key=True)
    data_sources = relationship("DataSourceGroup", back_populates="group",
                                 cascade="all")
    org_id = Column(db.Integer, db.ForeignKey('organizations.id'))
    ...
    # RBAC role type field
    role_type = Column(db.String(50), nullable=True)

    # Owner field
    owner_id = Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    owner = relationship("User", back_populates="owned_groups")

    # Relationship with DataSourceApproval
    data_source_approvals = relationship("DataSourceApproval", back_populates="group")
    ...
    def get_permissions_for_role_type(self):
        """
        Get permissions for this group's role type.
        
        Returns:
            list: List of permission strings for the role type, or empty list if no role type
        """
        if self.role_type and self.role_type in self.ROLE_PERMISSIONS:
            return self.ROLE_PERMISSIONS[self.role_type]
        return []

    @classmethod
    def all(cls, org):
        """Fetch all groups for a given organization."""
        return cls.query.filter(cls.org == org).all()
    ...
    def find_by_name(cls, org, group_names):
        """Find groups by their names within a specific organization."""
        return cls.query.filter(cls.org == org, cls.name.in_(group_names)).all()
