...
FRAME_OPTIONS = os.environ.get("REDASH_FRAME_OPTIONS", "allow")
...
CONTENT_SECURITY_POLICY = os.environ.get(
    ...
    "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-eval' 'unsafe-inline'; font-src 'self' data:; img-src 'self' http: https: data:; object-src 'none'; frame-ancestors * https:; frame-src * https:;"
)
...
default_query_runners = [
    ...
    'redash.query_runner.csvurl',
    'redash.query_runner.csv',
    'redash.query_runner.excel',
    'redash.query_runner.hopi',
    'redash.query_runner.external', 
]
...
# ============================================================================
# VDB Multi-Tenancy Settings
# ============================================================================

# Enable VDB multi-tenancy for organization-level data isolation
VDB_MULTI_TENANCY_ENABLED = parse_boolean(os.environ.get('VDB_MULTI_TENANCY_ENABLED', 'false'))

# Enable user-level VDB isolation (requires VDB_MULTI_TENANCY_ENABLED=true)
# When enabled, each user gets their own isolated VDB instead of sharing an organization VDB
USER_VDB_ISOLATION_ENABLED = parse_boolean(os.environ.get('USER_VDB_ISOLATION_ENABLED', 'false'))

# VDB template name (used as base for creating new VDBs)
VDB_TEMPLATE_NAME = os.environ.get('VDB_TEMPLATE_NAME', 'MyVDBTest')

# Teiid servlet URL for VDB management operations
TEIID_SERVLET_URL = os.environ.get('TEIID_SERVLET_URL', 'http://localhost:8095/TeiidExcelImporterTest')

# Teiid servlet API key for authentication
TEIID_SERVLET_API_KEY = os.environ.get('TEIID_SERVLET_API_KEY', None)

# VDB credential encryption key (defaults to DATASOURCE_SECRET_KEY)
VDB_SECRET_KEY = os.environ.get('VDB_SECRET_KEY', DATASOURCE_SECRET_KEY)

# Teiid server connection details
TEIID_HOST = os.environ.get('TEIID_HOST', 'localhost')
TEIID_PORT = int(os.environ.get('TEIID_PORT', '31020'))

# Enable SSL for Teiid connections
TEIID_USE_SSL = parse_boolean(os.environ.get('TEIID_USE_SSL', 'true'))

# Customer folder base path for VDB files and uploads
CUSTOMER_BASE_PATH = os.environ.get('CUSTOMER_BASE_PATH', '/opt/wildfly/teiidfiles/customers')

# Default VDB connection (fallback when VDB multi-tenancy is disabled)
# Note: Credentials should NOT be in this URL - they are passed separately
REDASH_VIRTUALDATABASE_URL = os.environ.get('REDASH_VIRTUALDATABASE_URL', 'jdbc:teiid:MyVDBTest@mm://localhost:31020')

# Admin notification emails for provisioning failures
# Comma-separated list of email addresses to notify on provisioning failures
PROVISIONING_FAILURE_NOTIFICATION_EMAILS = array_from_string(
    os.environ.get('REDASH_PROVISIONING_FAILURE_NOTIFICATION_EMAILS', '')
)
...
# Enable MFA for privileged users (Organization Admin and Super Admin roles)
MFA_ENABLED = parse_boolean(os.environ.get('REDASH_MFA_ENABLED', 'false'))
...
MFA_OTP_LENGTH = int(os.environ.get('REDASH_MFA_OTP_LENGTH', '6'))
MFA_OTP_EXPIRY_MINUTES = int(os.environ.get('REDASH_MFA_OTP_EXPIRY_MINUTES', '10'))
MFA_MAX_OTP_ATTEMPTS = int(os.environ.get('REDASH_MFA_MAX_OTP_ATTEMPTS', '5'))
MFA_LOCKOUT_MINUTES = int(os.environ.get('REDASH_MFA_LOCKOUT_MINUTES', '15'))

# MFA SMS Rate Limiting
MFA_MAX_SMS_REQUESTS_PER_PERIOD = int(os.environ.get('REDASH_MFA_MAX_SMS_REQUESTS_PER_PERIOD', '3'))
MFA_SMS_REQUEST_PERIOD_MINUTES = int(os.environ.get('REDASH_MFA_SMS_REQUEST_PERIOD_MINUTES', '10'))

# MFA Backup Codes Configuration
MFA_BACKUP_CODE_COUNT = int(os.environ.get('REDASH_MFA_BACKUP_CODE_COUNT', '10'))
MFA_BACKUP_CODE_WARNING_THRESHOLD = int(os.environ.get('REDASH_MFA_BACKUP_CODE_WARNING_THRESHOLD', '3'))


def is_mfa_configured():
    """
    Check if MFA is properly configured with all required settings.
    
    Returns:
        bool: True if MFA is enabled and Twilio credentials are configured
    """
    if not MFA_ENABLED:
        return False
...
def validate_vdb_configuration():
    """
    Validate VDB configuration settings and return warnings for missing or invalid settings.
    
    Returns:
        list: List of warning messages for configuration issues
    """
    warnings = []
    
    # Check if user VDB isolation is enabled without multi-tenancy
    if USER_VDB_ISOLATION_ENABLED and not VDB_MULTI_TENANCY_ENABLED:
        warnings.append(
            "USER_VDB_ISOLATION_ENABLED is set to true but VDB_MULTI_TENANCY_ENABLED is false. "
            "User VDB isolation requires VDB multi-tenancy to be enabled. "
            "Please set VDB_MULTI_TENANCY_ENABLED=true in your .env file."
        )
    
    # Check if VDB multi-tenancy or user isolation is enabled
    if VDB_MULTI_TENANCY_ENABLED or USER_VDB_ISOLATION_ENABLED:
        # Validate VDB_SECRET_KEY is set
        if not VDB_SECRET_KEY or VDB_SECRET_KEY == DATASOURCE_SECRET_KEY:
            warnings.append(
                "VDB_SECRET_KEY is not set or is using the default DATASOURCE_SECRET_KEY. "
                "For security, please generate a unique encryption key for VDB credentials. "
                "Generate one with: python -c \"from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())\""
            )
        
        # Validate TEIID_SERVLET_URL is set
        if not TEIID_SERVLET_URL or TEIID_SERVLET_URL == 'http://localhost:8095/TeiidExcelImporterTest':
            warnings.append(
                "TEIID_SERVLET_URL is not configured or is using the default value. "
                "Please set TEIID_SERVLET_URL to your Teiid servlet endpoint in your .env file. "
                "Example: TEIID_SERVLET_URL=https://teiid-server.internal:8095/TeiidExcelImporterTest"
            )
        
        # Validate TEIID_SERVLET_API_KEY is set
        if not TEIID_SERVLET_API_KEY:
            warnings.append(
                "TEIID_SERVLET_API_KEY is not set. "
                "Please generate and set an API key for Teiid servlet authentication. "
                "Generate one with: python -c \"import secrets; print(secrets.token_urlsafe(32))\""
            )
        
        # Warn if using HTTP instead of HTTPS in production
        if TEIID_SERVLET_URL and TEIID_SERVLET_URL.startswith('http://') and not TEIID_SERVLET_URL.startswith('http://localhost'):
            warnings.append(
                "TEIID_SERVLET_URL is using HTTP instead of HTTPS. "
                "For production environments, please use HTTPS to secure VDB management operations."
            )
    
    return warnings


def is_user_vdb_isolation_configured():
    """
    Check if user VDB isolation is properly configured with all required settings.
    
    Returns:
        bool: True if user VDB isolation is enabled and properly configured
    """
    if not USER_VDB_ISOLATION_ENABLED:
        return False
    
    # User VDB isolation requires multi-tenancy to be enabled
    if not VDB_MULTI_TENANCY_ENABLED:
        return False
    
    # Check that required settings are configured
    return bool(
        VDB_SECRET_KEY and 
        VDB_SECRET_KEY != DATASOURCE_SECRET_KEY and
        TEIID_SERVLET_URL and 
        TEIID_SERVLET_API_KEY
    )
