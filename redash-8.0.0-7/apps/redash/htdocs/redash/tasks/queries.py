...
class QueryExecutor(object):
    ...
    def run(self):
        ...
        # Pass query_id to query runner for VDB routing (needed for shared projects)
        if self.metadata and 'Query ID' in self.metadata:
            query_runner._query_id = self.metadata['Query ID']
            logger.debug("Set query_id on query_runner: {}".format(query_runner._query_id))
