...
`Sidebar` (`sidebar.tsx`) renders the same core nav — Home, Business Insight, Projects, AI Assistant — in both Home and Project mode; only the selector pill at the top (tenant vs. active project) and what appears below the core nav differ:
