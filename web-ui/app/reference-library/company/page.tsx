...
export default function CompanyLibraryPage() {
  ...
  async function toggleInherit(doc: ReferenceDocument) {
    await referenceLibraryApi.updateDocument(doc.id, { inherit_default: !doc.inheritDefault });
    void load();
  }
  ...
  return (
    <AppShell
      mode="home"
      activeNav="company-reference-library"
      tenant={tenant}
      user={user}
      counts={{ projects: projects?.length }}
      centered
      topBarLeft={<span className="text-h2 text-ink-primary">Company Reference Library</span>}
      topBarRight={
        canWrite ? (
    ...
    >
      <div className="space-y-4">
        <p className="text-[13px] text-ink-secondary">
          Internal policies and standards for your company. Visible only within your tenant.
          Toggle <strong>Inherit by default</strong> to auto-include a document in every
          project&apos;s library.
        </p>
