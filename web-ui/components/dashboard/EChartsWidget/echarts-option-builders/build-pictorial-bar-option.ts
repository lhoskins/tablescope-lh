...
export function buildPictorialBarOption(
  ...
  return {
    ...
    tooltip: opts.showTooltip === false ? { show: false } : { trigger: "axis" as const, formatter: (p: any) => `${p[0]?.name}<br/>${yKey}: ${formatNumber(Number(p[0]?.value ?? 0), opts.yAxisFormat, undefined, opts.currencySymbol)}` },
    ...
    series: [{
      ...
      label: { show: !!opts.showLabels, position: "top", fontSize: 9, color: colorsForChart.text, formatter: (p: any) => formatNumber(Number(p.value ?? 0), opts.yAxisFormat, undefined, opts.currencySymbol) },
    }],
  };
}