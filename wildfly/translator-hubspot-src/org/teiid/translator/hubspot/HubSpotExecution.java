...
/**
 * Executes a Teiid {@link Select} against the HubSpot CRM v3 Objects API.
 *
 * Pushes down table, column projection and row limit.  Unsupported predicates
 * are omitted; Teiid's engine filters the returned rows in memory.
 */
public class HubSpotExecution implements ResultSetExecution {
    ...
    public HubSpotExecution(Select command, HubSpotConnection connection, ExecutionContext context, RuntimeMetadata metadata) {
    ...
    public void execute() throws TranslatorException {
        ...
        for (JsonObject row : connection.query(sourceTable, fields, limit, 0)) {
    ...
    private Object getCell(JsonObject row, String src) {
        ...
        JsonValue top = row.get(src);
        if (top != null && top != JsonValue.NULL) {
            return convertValue(top);
        }
        JsonObject props = row.getJsonObject("properties");
        if (props != null && props.containsKey(src)) {
            JsonValue v = props.get(src);
            // HubSpot sometimes returns {"value": "..."} wrappers.
            if (v != null && v.getValueType() == JsonValue.ValueType.OBJECT) {
                JsonObject vo = (JsonObject) v;
                if (vo.containsKey("value")) {
                    return convertValue(vo.get("value"));
                }
            }
